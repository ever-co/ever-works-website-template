...
- `spec-047`: email two-factor authentication for client accounts — enable/disable card on `/client/settings/security`, hashed six-digit code emailed on every credentials sign-in, 10-minute expiry with resend, 5-failure / 15-minute database-tracked lockout, and OAuth-only accounts refused in the UI and at the API ([spec 047](spec/047-email-two-factor-auth/spec.md), [plan](spec/047-email-two-factor-auth/plan.md), [tasks](spec/047-email-two-factor-auth/tasks.md), Jira EW-135 … EW-142, PR #1048).
